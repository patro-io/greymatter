...
class MemoryQueries {
  ...
  // opts.pathPrefixes: optional array of absolute path prefixes. When given, a window
  //   is kept only if it touched a file under one of them — i.e. the conversation was
  //   about that project.
  // Returns windows with scores and decision digests, sorted by score desc.
  //
  // Why paths and not sessions.projects_json: that column exists but is empty for every
  // session on record (903/903 measured 2026-09-14), so filtering on it would hide
  // everything. window_files carries ~21k absolute paths and is the signal that is
  // actually there. Recall spans every project, so without a filter a generic term from
  // a sprint slug ("gate", "plugin") routinely surfaces decisions from a different repo
  // — 13 such digests in one devstack orientation.
  searchConversations(clusters, limit, opts = {}) {
    const pathPrefixes = (opts.pathPrefixes || []).filter(Boolean);
    ...
    // Prefix match on the stored absolute path. Anchored with LIKE 'prefix/%' so
    // /home/github/patro never matches /home/github/patro-cms.
    const touchesProject = pathPrefixes.length
      ? this._db.db.prepare(
          'SELECT 1 FROM window_files WHERE window_id = ? AND (' +
          pathPrefixes.map(() => 'file_path LIKE ?').join(' OR ') +
          ') LIMIT 1')
      : null;
    const getDecisions = this._db.db.prepare(
    ...
    for (const id of candidateIds) {
      ...
      if (!win) continue;
      if (touchesProject && !touchesProject.get(id, ...pathPrefixes.map(p => p + '/%'))) continue;
